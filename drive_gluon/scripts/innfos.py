...
def setspeed(genid,speed):
    indg = 0
    for byteg in genid:
            # set maximum speed
            if speed[indg] < 0:
                speedg = int((256 + speed[indg])*2**24).to_bytes(4,'big')
            else:
                speedg = int(speed[indg]*2**24).to_bytes(4,'big')
            CRCg = CRC16_1(speedg)
            s.sendto(bytes([0xEE, byteg, 0x09, 0x00, 0x04, speedg[0], speedg[1], speedg[2], speedg[3], CRCg[0], CRCg[1], 0xED]),(UDP_IP, UDP_PORT))
            indg = indg + 1
    return 1
...
def readspd(genid):
    ...
    spdgen =[]
    for byteg in genid:
        s.sendto(bytes([0xEE, byteg, 0x05, 0x00, 0x00, 0xED]),(UDP_IP, UDP_PORT))
        data = s.recvfrom(BUFFER_SIZE)
        gtempg = int.from_bytes(data[0][5:7],'big')/(2**24) # i124
        spdgen.append(gtempg)
    return spdgen
